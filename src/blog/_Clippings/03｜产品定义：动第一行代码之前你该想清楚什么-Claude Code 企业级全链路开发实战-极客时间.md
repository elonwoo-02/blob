---
title: "03｜产品定义：动第一行代码之前你该想清楚什么-Claude Code 企业级全链路开发实战-极客时间"
source: "https://time.geekbang.org/column/article/963507"
author:
published:
created: 2026-05-12
description: "本文总结了在产品定义之前，了解Dify的核心功能模块，使用“三问裁剪法”确定Hify的功能清单。技术选型考虑匹配定位、生态成熟度、AI领域SDK支持、运维复杂度。最终选定Spring Boot + Vue技术栈。运维预期包括Docker Compose部署、QPS估算、缓存策略、数据持久化。整个过程强调了问题的鉴别和解决，以及技术选型和运维预期的重要性。"
tags:
  - "clippings"
---
Robert · Claude Code 企业级全链路开发实战
...
![](https://static001.geekbang.org/resource/image/32/be/32948aedbbb71d51acf81e8c394a0ebe.png?wh=1274x1576)
...
![](https://static001.geekbang.org/resource/image/b7/65/b76d2325df3c3b1389c71770a743f965.jpg?wh=1382x376)
...
![](https://static001.geekbang.org/resource/image/1c/47/1ca77f1178fc6737aff60862af5f3e47.png?wh=1512x1440)
...
![](https://static001.geekbang.org/resource/image/24/c1/24e85aa9066f10f5fd27b756baf816c1.png?wh=2470x1478)

你会发现结论很明确：Python FastAPI 在 AI 生态上碾压其他两个——LangChain、LlamaIndex，向量库客户端都是 Python 原生的，Java 和 Go 在这方面差距很大。它推荐 FastAPI，还给了一个折中方案：Spring Boot 做业务编排，AI 重逻辑（RAG、向量化）抽成一个 Python 小服务。 你会发现结论很明确：Python FastAPI 在 AI 生态上碾压其他两个——LangChain、LlamaIndex，向量库客户端都是 Python 原生的，Java 和 Go 在这方面差距很大。它推荐 FastAPI，还给了一个折中方案：Spring Boot 做业务编排，AI 重逻辑（RAG、向量化）抽成一个 Python 小服务。
...
AI SDK 用 Java 版本够用，OpenAI、Claude 都有 Java SDK，RAG 的向量化调 API 就行，不依赖 LangChain。 AI SDK 用 Java 版本够用，OpenAI、Claude 都有 Java SDK，RAG 的向量化调 API 就行，不依赖 LangChain。
...
后端：Spring Boot 3.x + MyBatis-Plus + MySQL 8.x + Redis 7.x。工程化成熟，一个人能快速交付完整系统。 后端：Spring Boot 3.x + MyBatis-Plus + MySQL 8.x + Redis 7.x。工程化成熟，一个人能快速交付完整系统。
...
SSE 长连接是唯一需要认真对待的压力点。50 个并发 SSE 连接，每个持续 10-30 秒，意味着 50 个线程被长期占用。Spring Boot 默认 200 线程够用，但要注意：用 SseEmitter 不要用普通 ResponseBody 阻塞线程，设 60 秒超时防止僵尸连接，Nginx 反向代理缓冲（proxy\_buffering off）否则流式数据会被攒批再发。这几个坑我自己没全想到，尤其是 Nginx 缓冲那个——不关的话用户看不到打字机效果，要等 LLM 全部输出完才一次性吐出来。
...
监控：起步用 Spring Boot Actuator + 日志，后期接 Prometheus + Grafana。 监控：起步用 Spring Boot Actuator + 日志，后期接 Prometheus + Grafana。
